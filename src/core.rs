...
const SPAWN_ICONS_PATH: &str = "lua/autorun/client/gmm_spawn_icons.lua";
const SPAWN_ICONS_SCRIPT: &str = include_str!("spawn_icons.lua");
...
pub fn build_preview(
    ...
    if manage_addons {
        ...
        add_change(
            ...
            &game,
            SPAWN_ICONS_PATH,
            SPAWN_ICONS_SCRIPT.as_bytes().to_vec(),
        )?;
    ...
    if !manage_addons {
        ...
        if game.join(SPAWN_ICONS_PATH).exists() {
            add_change(&mut preview, &game, SPAWN_ICONS_PATH, Vec::new())?;
        }
    }
